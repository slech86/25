...
class RegistrationPage(BasePage):
    def filling_in_required_fields(self):  # заполнение обязательных полей
        if "ua" in self.browser.current_url:
            self.browser.find_element(*RegistrationPageLocators.FIELD_LOGIN).send_keys(TestData.login_ua)
        else:
            self.browser.find_element(*RegistrationPageLocators.FIELD_LOGIN).send_keys(TestData.login_ru)

        if "ua" in self.browser.current_url:
            self.browser.find_element(*RegistrationPageLocators.FIELD_EMAIL).send_keys(TestData.email_ua)
        else:
            self.browser.find_element(*RegistrationPageLocators.FIELD_EMAIL).send_keys(TestData.email_ru)

        self.browser.find_element(*RegistrationPageLocators.FIELD_PASSWORD).send_keys(TestData.password)
        self.browser.find_element(*RegistrationPageLocators.FIELD_REPEAT_PASSWORD).send_keys(TestData.password)
        # заполнение блока "Данные для авторизации"

        self.browser.find_element(*RegistrationPageLocators.FIELD_NAME).send_keys('name' + TestData.time_Now)
        self.browser.find_element(*RegistrationPageLocators.FIELD_SURNAME).send_keys('surname' + TestData.time_Now)
        self.browser.find_element(*RegistrationPageLocators.FIELD_POSITION).send_keys('position' + TestData.time_Now)
        self.browser.find_element(*RegistrationPageLocators.FIELD_TELEPHONE).send_keys('+01(010)101-01-01' + TestData.time_Now)
        self.browser.find_element(*RegistrationPageLocators.FIELD_CONTACT_EMAIL).send_keys('contact_email' + TestData.time_Now + TestData.email[1])
        # заполнение блока "Контактная информация"

        self.browser.find_element(*RegistrationPageLocators.FIELD_COMPANY_NAME).send_keys('company_name' + TestData.time_Now)
    ...
    def filling_in_optional_fields(self):  # заполнение не обязательных полей
        self.browser.find_element(*RegistrationPageLocators.FIELD_SKYPE).send_keys('skype' + TestData.time_Now)
        # заполнение блока "Контактная информация"
        ...
        self.browser.find_element(*RegistrationPageLocators.FIELD_STREET).send_keys('street' + TestData.time_Now)
        ...
        self.browser.find_element(*RegistrationPageLocators.FIELD_COMPANY_SITE).send_keys('http://company_site' + TestData.time_Now + '.com')
        ...
        self.browser.find_element(*RegistrationPageLocators.FIELD_FACEBOOK).send_keys('http://facebook' + TestData.time_Now + '.com')
        self.browser.find_element(*RegistrationPageLocators.FIELD_LINKEDIN).send_keys('http://linkedin' + TestData.time_Now + '.com')
        self.browser.find_element(*RegistrationPageLocators.FIELD_INSTAGRAM).send_keys('http://instagram' + TestData.time_Now + '.com')
        self.browser.find_element(*RegistrationPageLocators.FIELD_TELEGRAM).send_keys('http://telegram' + TestData.time_Now + '.com')
        self.browser.find_element(*RegistrationPageLocators.FIELD_TWITTER).send_keys('http://twitter' + TestData.time_Now + '.com')
        self.browser.find_element(*RegistrationPageLocators.FIELD_VK).send_keys('http://vk' + TestData.time_Now + '.com')
